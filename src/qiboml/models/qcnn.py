...
class QuantumCNN:
    ...
    def __init__(
        ...
        Args:
            ...
            fullstate: Default to None. True means we ignore the measurement gates
        ...
        if self.quantum_model == 'QCNNCOMPLEX':
            self.nparams_conv = 15   # 4 Rotation of 3 angles and one RXX RYY RZZ of 3 angles. Total: 5*3=15
            self.nparams_pool = 6  # 2 sets of rotation angles. These two sets is with CNOT, and the bottom one (sink) carries out R^{\dagger}(vectheta_2).
        ...
        nparams = self.nparams_layer * nlayers
        self.nparams = nparams

        if fullstate:
            params = [i+1 for i in range(nparams) ] # this set [1, 2, 3, ..., nparams]
        self._circuit = self.ansatz(nlayers, params=params,fullstate=fullstate)
        if fullstate: 
            # overwrite previously assign params
            params_indexed = self._circuit.get_parameters()
            nparams_expanded = len(params_indexed) # set of expanded params
            self.nparams_expanded = nparams_expanded
            idxs = [  int(v[0]) for v in params_indexed ]
            if -1 in idxs:
                assert False, 'we hit ambiguous sign!'
            assert all(  0 < abs(x) <= nparams for x in idxs), f'Index out of bound: idxs={idxs}'
            f_idxs = [ abs(float(v[0])) for v in params_indexed ]
            i_idxs = [ abs(v) for v in idxs]
    ...
    def ansatz(self, nlayers, params=None,fullstate=None):
        """
        Args:
