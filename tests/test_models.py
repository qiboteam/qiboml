...
def test_phase_encoding_layer():
    nqubits = 10
    qubits = np.random.choice(range(nqubits), size=(6,), replace=False)
    layer = ed.PhaseEncodingLayer(nqubits, qubits=qubits)
...
def test_probabilities_layer():
    nqubits = 5
    qubits = np.random.choice(range(nqubits), size=(4,), replace=False)
    layer = ed.ProbabilitiesLayer(nqubits, qubits=qubits)
...
def test_state_layer():
    nqubits = 5
    layer = ed.StateLayer(nqubits)
    c = random_clifford(nqubits)
    np.testing.assert_allclose(layer(c), c().state())
